  ...
  export default function UserProfilePage() {
    ...
    useEffect(() => {
      const fetchOwnerData = async () => {
        ...
        if (token) {
          try {
            // Fetch owner data using the ownerId from the URL
    ...
    const updateOwner = async () => {
      try {
        const response = await axios.put(`http://localhost:5000/api/owners/${owner.id}`, owner); // Update the URL as needed
        setOwner(response.data); // Update the owner state with the response data
    ...
    const handleSave = () => {
      updateOwner(); // Call the update function
